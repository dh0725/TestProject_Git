// 기본 데이터
var page = 1; // 현재 페이지
var pageRows = 10; // 페이지당 글의 갯수
var viewItem = undefined; // 가장 최근에 view한 글의 데이터

// 페이지가 최초로 로딩되면 게시글 목록의 첫 페이지를 로딩하게 하기
$(document).ready(function() {
    loadPage(page); // 페이지가 최초로 로딩되었을때
    ...
    // 글 읽기(view) 대화상자에서 삭제버튼 누르면 해당 글 삭제 진행
...
function loadPage(page) {
    $.ajax({
        ...
        success: function(data, status) {
            if (status == "success") {
                // alert("성공");
                if (updateList(data)) { // application/json이면 이미 json으로 파싱되어 있다.

                    // 업데이트 된 list에 view 동작 이벤트가동
                    addViewEvent();
                    // 만약 위 코드를 $(document).ready()에 두면 동작 안핡것이다!
                }
            }
         }
    }); // end $.ajax()
...
function updateList(jsonObj) {
    ...
    if (jsonObj.status == "OK") {
        ...
        $("#list tbody").html(result); // 위의 데이터로 업데이트

        // 페이지 정보 업데이트
        $("#pageinfo").text(jsonObj.page + "/" + jsonObj.totalpage + "페이지, " + jsonObj.totalcnt + "개의 글");

        // pageRows 한 페이지에 몇개의 글을 불러올지?
        var txt = "<select id='rows' onchange='changePageRows()'>\n";

        txt += "<option " + ((window.pageRows == 10) ? "selected" : "") + " value='10'>" + "10개 씩" + "</option>\n";
        txt += "<option " + ((window.pageRows == 20) ? "selected" : "") + " value='20'>" + "20개 씩" + "</option>\n";
        txt += "<option " + ((window.pageRows == 50) ? "selected" : "") + " value='50'>" + "50개 씩" + "</option>\n";
        txt += "<option " + ((window.pageRows == 100) ? "selected" : "") + " value='100'>" + "100개 씩" + "</option>\n";
        ...
        return true; // 목록 업데이트에 성공하면 true를 리턴하기
    } else {
        alert("내용이 없습니다.");
        return false;
    }
